...
public class Trick implements Serializable {
	 ...
	 *
	 */
	private static final long serialVersionUID = -3571722184668406412L;
	...
	public void play(Player p, Card c) throws IllegalMove {
		...
		if (p.direction.equals(getNextToPlay())) {//check if it is your turn
			if (p.hand.contains(c)) {//check if the card is in your hand
				if (!getCardsPlayed().isEmpty()) {//check if there are cards played
					for (Card e : p.hand) {//check every card in your hand see if there is same suit card
						if (e.suit().equals(getCardsPlayed().get(0).suit())) {
							hasSuit = true;//if yes, make hasSuit be true
						}
					}
					if ((hasSuit && c.suit().equals(getCardsPlayed().get(0).suit())) || !hasSuit) {
						for (int i = 0; i != 4; ++i) {
							if (cards[i] == null) {
								cards[i] = c;
								p.getHand().remove(c);
				...
				throw new IllegalMove("No this card!");
		} else
